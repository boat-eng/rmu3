...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['do_pic'])) {
    ...
    if (isset($_FILES['profile_pic']) && $_FILES['profile_pic']['error'] === UPLOAD_ERR_OK) {
        ...
        if (!in_array($ftype, $allowed)) {
            ...
            // Delete old pic
            $old = $pdo->prepare('SELECT profile_pic FROM users WHERE id=?');
            $old->execute([$uid]);
            ...
            $pdo->prepare('UPDATE users SET profile_pic=? WHERE id=?')->execute([$filename, $uid]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['do_update'])) {
    ...
    if (!$full_name) {
        ...
        $pdo->prepare('UPDATE users SET full_name=?, department=?, bio=? WHERE id=?')
            ->execute([$full_name, 'Department of ICT', $bio, $uid]);
...
// Stats
$nEnrolled = $pdo->prepare('SELECT COUNT(*) FROM courses WHERE instructor_id=?');
$nEnrolled->execute([$uid]); $nEnrolled = (int)$nEnrolled->fetchColumn();

$nDone = $pdo->prepare('SELECT COUNT(DISTINCT e.student_id) FROM enrollments e JOIN courses c ON c.id=e.course_id WHERE c.instructor_id=?');
    ...
    <div>
        <div class="pcard">
            ...
            <div class="pinfo">
                ...
                <div class="prole"><i class="fas fa-chalkboard-teacher"></i>&nbsp; Instructor</div>
